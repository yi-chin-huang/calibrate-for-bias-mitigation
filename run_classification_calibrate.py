...
def main(models, datasets, all_shots, num_seeds, subsample_test_set, api_num_log_prob, approx, use_saved_results, bs, calibrate):
    """
    ...
    if use_saved_results:
        ...
        save_results(all_params, calibrate)
    else:
        save_results(all_params)


def save_results(params_list, calibrate = False, freeze_test_set=True):
    """
    ...
    for param_index, params in enumerate(params_list):
        ...
        # uncomment to output csv (make sure you name it properly!)
        # output_csv(sentences, templates, emotions, genders, races, predicted_intensities, model=params['model'])

        # get prob for each label
        if calibrate == True:
            print("CALIBRATING...")
            all_label_probs = get_label_probs(params, raw_resp_test, 
                                              train_sentences=['The conversation with Latisha was displeasing.'], 
                                              train_labels=[2], test_sentences=sentences, test_emotions=emotions)

            # calculate P_cf
            bias_inputs = ["african american"]
            p_cf = get_p_bias(params, ['The conversation with Latisha was displeasing.'], [2], bias_inputs=bias_inputs)
...
def get_label_probs(params, raw_resp, train_sentences, train_labels, test_sentences, test_emotions):
...
def get_p_bias(params, train_sentences, train_labels, bias_inputs=('N/A',)):
    """Query model with content free input, return its prediction probability for each label"""
    label_dict = params['label_dict']

    all_p_y = []
    for bias_input in bias_inputs:
        prompt = construct_prompt(params, train_sentences, train_labels, bias_input)
...
if __name__ == '__main__':
    ...
    # calibrate
    parser.add_argument('--calibrate', dest='calibrate', action="store_const", const=True, default=False,
                        help="whether to apply calibration layer")
